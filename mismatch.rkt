...
(define label-picts (get-label-picts min-mm max-mm (add1 (length color-list))))
(define color-picts (get-color-picts color-list))
(define pad-below (ghost (rectangle 60 33)))
(define colorbar (vl-append
                  (hc-append 1 (apply vl-append (reverse color-picts))
                             (apply vl-append (reverse label-picts)))
                  pad-below))
(define scatter+colorbar (hc-append 10 scatter colorbar))
(save-pict-as-pdf scatter+colorbar #:file-name "scatter-colorbar.pdf" #:adjust-width 70)