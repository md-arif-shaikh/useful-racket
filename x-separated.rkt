...
;; written out.
(define (write-xsv df outp series #:start start #:stop stop #:sep (sep #\,))
  (define first? #t)
  (define columns (if (null? series) (df-series-names df) series))
  (for ([header (in-list columns)])
    (if first?
        (set! first? #f)
        (write-string sep outp))
...
;; written out.
(define (df-write/xsv df outp #:start (start 0) #:sep (sep #\,) #:stop (stop (df-row-count df)) . series)
  (if (path-string? outp)
      (call-with-output-file outp
        #:mode 'text #:exists 'truncate/replace
        (lambda (o)
          (write-xsv df o series #:start start #:stop stop #:sep sep)))
      (write-xsv df outp series #:start start #:stop stop #:sep sep)))

(define df (make-data-frame))
(define time (build-list 10 values))
(df-add-series! df (make-series "time" #:data (list->vector time)))
(df-add-series! df (make-series "alphabet" #:data (list->vector (map sqrt time))))
(df-write/xsv df "test-tab-tab.csv" #:sep "\t\t")
...
(define (parse-line in quoted-numbers? sep)
...
(define (read-xsv input headers? na qn? sep)
  ...
  (unless (eof-object? (peek-char input))
    ...
    (let row-loop ()
      (unless (eof-object? (peek-char input))
        (let-values ([(cells cell-count) (parse-line input qn? sep)])
...
;; available value).
(define (df-read/xsv inp #:headers? (headers? #t) #:na (na "") #:quoted-numbers? (qn? #f) #:sep (sep #\,))
  (if (path-string? inp)
      ;; not 'text: we might read MAC text files on a Windows machine!
      (call-with-input-file inp #:mode 'text
        (lambda (i) (read-xsv i headers? na qn? sep)))
      (read-xsv inp headers? na qn?)))
(provide df-read/xsv)
(provide df-write/xsv)
